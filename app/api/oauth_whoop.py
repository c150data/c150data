...
def getAndInsertNewToken(whoopAthleteId, whoopUsername, whoopPassword):
    ...
    if response.status_code != 200:
        raise Exception("Error getting a new token for athlete {}. Could be they changed their password or username.".format(whoopAthleteId))
...
def getInfoForNewAthlete(username, password):
    ...
    if response.status_code != 200:
        raise Exception("Error getting information for new whoop athlete. Possibly incorrect username or password.")
